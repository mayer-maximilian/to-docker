...
export default {
    ...
    methods: {
        getUsers () {
            ...
            if (jwt_token) {
                axios.get(`${!getEnv('ENV') ? 'http://todo-api-service' : ''}/users/get-users`, {'headers': {'Authorization': `bearer ${jwt_token}`}})
                .then((response) => {
        ...
        editItem () {
            let jwt_token = getCookie("jwt")
            axios.post(`${!getEnv('ENV') ? 'http://todo-api-service' : ''}/users/update-user`, this.editing_user, {'headers': {'Authorization': `bearer ${jwt_token}`}})
            .then(() => {
                this.getUsers()
                this.onReset()
            })
        ...
        changePassword () {
            ...
            axios.post(`${!getEnv('ENV') ? 'http://todo-api-service' : ''}/users/change-password`, data, {'headers': {'Authorization': `bearer ${jwt_token}`}})
            .then(() => {
        ...
        submitNewUser () {
            let jwt_token = getCookie("jwt")
            axios.post(`${!getEnv('ENV') ? 'http://todo-api-service' : ''}/users/add-user`, this.adding_user, {'headers': {'Authorization': `bearer ${jwt_token}`}})
            .then(() => {
                this.getUsers()
                this.onReset()
            })
        ...
        deleteUser () {
            let jwt_token = getCookie("jwt")
            axios.post(`${!getEnv('ENV') ? 'http://todo-api-service' : ''}/users/delete-user?username=${this.editing_user.username}`, {}, {'headers': {'Authorization': `bearer ${jwt_token}`}})
            .then(() => {
                this.getUsers()
                this.onReset()
            })
